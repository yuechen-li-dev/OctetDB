...
func openBSO(ctx context.Context, root, id string, balance Money, m *Metrics) (*durableBSO, error) {
...
func (b *durableBSO) open(ctx context.Context) error {
	...
	state, err := bucket.Dataset(ctx, "state", octetdb.DatasetOptions{TypeIdentity: "bso-sim-m1.BSOState/v1"})
	if err != nil {
		_ = db.Close()
		return err
	}
