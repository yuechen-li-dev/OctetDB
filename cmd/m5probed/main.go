...
func main() {
	start := time.Now()
	row, ok := (m5compiled.ScanLane{}).FindByID(m5.FirstProductID + 417)
	if !ok {
		panic("compiled D first query failed")
	}
	ready := time.Since(start).Nanoseconds()
	count, digest := m5.DigestProducts((m5compiled.ScanLane{}).FindByCategory(3))
	priceCount, priceDigest := m5.DigestProducts((m5compiled.ScanLane{}).ProductsWithPriceBetween(50_000, 80_000))
	projectionCount, projectionDigest := m5.DigestProjections((m5compiled.ScanLane{}).ActiveProductsInRegion(2))
