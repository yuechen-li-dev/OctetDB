...
import (
	...
	"flag"
	"fmt"
	bsosim "github.com/yuechen-li-dev/octetdb/experiments/BSOSim/M1"
	"os"
	"time"
)
...
func main() {
	...
	if *mode == "smoke" {
		...
		base.BSOs, base.Transfers = 100, 300
	}
	if *workers > 0 {
	...
	fmt.Printf("BSO-SIM-M1 mode=%s runtime=%s\n", *mode, time.Since(start))
	fmt.Println("| Workers | Transfers | Throughput | Avg agent steps/worker | Max | Coordinator ops/success | Correct |")
	for _, r := range results {
		...
		fmt.Printf("| %d | %d | %.0f | %.1f | %d | %.2f | %t |\n", r.Config.Workers, r.Config.Transfers, r.Metrics.TransfersPerSecond, float64(r.Metrics.AgentSteps)/float64(r.Config.Workers), max, r.Metrics.CoordinatorOpsPerSuccess, r.Correct)
	}
}
