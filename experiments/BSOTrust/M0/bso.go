...
func openBSO(ctx context.Context, root, id string, balance Money, policy TrustPolicyV1, m *metricStore) (*durableBSO, error) {
	...
	initial := BSOState{ID: id, Balance: balance, Outgoing: map[string]Transfer{}, Incoming: map[string]Transfer{}, ProtocolVersion: 1, TrustPolicy: policy}
...
func (b *durableBSO) open(ctx context.Context) error {
	...
	state, err := bucket.Dataset(ctx, "state", octetdb.DatasetOptions{TypeIdentity: "bso-trust-m0.BSOState/v1"})
	if err != nil {
		_ = db.Close()
		return err
	}
...
// trustPolicy exposes only local trust policy to the protocol layer. Providers
// never receive the BSOState returned by load.
func (b *durableBSO) trustPolicy(ctx context.Context) (TrustPolicyV1, error) {
	state, err := b.load(ctx)
	if err != nil {
		return TrustPolicyV1{}, err
	}
	return state.TrustPolicy, nil
}

func (b *durableBSO) replaceTrustPolicy(ctx context.Context, policy TrustPolicyV1) error {
	if policy.BSOID != b.id {
		return errors.New("trust policy owner mismatch")
	}
	if err := validatePolicy(policy); err != nil {
		return err
	}
	_, err := b.mutate(ctx, "trust-policy/"+strconv.Itoa(policy.Version), func(state *BSOState) mutationResult {
		if policy.Version > state.TrustPolicy.Version {
