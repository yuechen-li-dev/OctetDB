// Package bsosim implements BSO-TRUST-M0, a bounded federated-trust extension
// of the BSO-SIM-M2 settlement experiment. It is not a payment product.
package bsosim
...
const (
	...
	PhaseTrustCollect     AgentPhase = "trust_collect"
	PhaseTrustAdmitted    AgentPhase = "trust_admitted"
...
type BSOState struct {
	...
	TrustPolicy     TrustPolicyV1       `json:"trust_policy"`
}
...
type TransactionAgent struct {
	...
	ApplicationReference, TrustResolutionID, TrustFailureReason                                                  string
	RequiredRoles                                                                                                []TrustRole
	TrustThresholds                                                                                              []int
	TrustCandidates, SelectedProviders, CollectedAttestationIDs                                                  []string
	TrustProviderIndex, SenderPolicyVersion, ReceiverPolicyVersion                                               int
	TrustProvidersConsulted, FreshProviderCalls, ReusedTrustAttestations                                         int
	CompletedRound                                                                                               int
}
...
type Metrics struct {
	...
	WorkerActiveCurrent, WorkerActivePeak, GoroutinesIdle, GoroutinesPeak                                                                               int
	BSOMutations, TransferCompletions, Retries                                                                                                          int
	WorkerGoroutinesStarted, WorkerGoroutinesStopped, GoroutinesShutdown                                                                                int
	ProvidersConsulted, AttestationsIssued, AttestationsUsed, ReusedAttestations, FallbackProviderUses, PolicyIntersectionFailures                      int
	ExpiredAttestationsRejected, DuplicateAttestationsSuppressed, TrustResolutions, TrustAdmitted, TrustRejected, SettlementBeforeTrust                 int
}
...
type Result struct {
	...
	TrustResolutions         []TrustResolutionV1   `json:"trust_resolutions"`
	ProviderRetention        []ProviderRetentionV1 `json:"provider_retention"`
	CoordinationCosts        []CoordinationCostV1  `json:"coordination_costs"`
}

type CoordinationCostV1 struct {
	TransferID                                                                                                  string `json:"transfer_id"`
	SettlementMessages, ProviderCalls, FinancialDurableTransitions, ResolutionDurableTransitions, LogicalRounds int
}

func DefaultConfig() Config {
	return Config{Seed: 20260823, BSOs: 8, Transfers: 10, Workers: 4, InitialBalance: 100_000, Workload: "trust-suite", Faults: FaultProfiles["none"], MaxRounds: 96, RetryDelay: 2, ReservationExpiry: 24, KillWorker: -1}
}
